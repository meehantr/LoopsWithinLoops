...
def multiply_numbers(sequence_of_lists):
    ...
    for i in range(len(sequence_of_lists)):
        for j in range(len(sequence_of_lists[i])):
            current_list = sequence_of_lists[i]
            current_list[j] = current_list[j] * (i + 1)
...
def run_test_sum_numbers():
    ...
    # Test 3:
    expected = 36
    answer = sum_numbers(([4, 5], [5, 3, 1], [7, 6, 5]))
    print('Expected and actual are:', expected, answer)
...
def print_characters(sequence_of_strings):
    ...
    for i in range(len(sequence_of_strings)):
        for j in range(len(sequence_of_strings[i])):
            inner_string = sequence_of_strings[i]
            print(inner_string[j])
